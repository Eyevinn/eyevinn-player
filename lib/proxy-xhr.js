(function(videojs){
   ...
   */
   videojs.Hls.xhr = function(url, callback) {
    ...
    var proxy = require('url').parse(url);
    if (proxy.query && !proxy.query.match(/hostname=/)) {
      proxy.query = proxy.query + '&hostname=' + proxy.hostname;
    } else {
      proxy.query = '?hostname=' + proxy.hostname;
    }
    if (proxy.port) {
      proxy.query = proxy.query + '&port=' + proxy.port;
    } else {
      proxy.query = proxy.query + '&port=0';
    }
    proxy.search = proxy.query;
